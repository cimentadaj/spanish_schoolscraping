...
For web scraping in `R`, you can fulfill almost all of your needs with the `xml2` and `httr` package. As you wander through the web, you'll see many examples using the `rvest` package. `xml2` and `rvest` are very similar so don't feel you're lacking behind for learning one and not the other.

In addittion to these two packages, we'll need some others for plotting locations on a map (`ggplot2`, `sf`, `rnaturalearth`) and wrangling data (`tidyverse`). Let's load all of them:
...
Let's begin with a simple example. Below we define an XML structure and look at it's structure:
...
knitr::include_graphics("./xml_examples/xml_one.png")
```
...
```{r, out.width = "30%", echo = FALSE}
knitr::include_graphics("./xml_examples/xml_two.png")
```

Similarly, you'll find that the `<carol>` tag is also matched by a `</carol>` finishing tag.

```{r, out.width = "30%", echo = FALSE}
knitr::include_graphics("./xml_examples/xml_three.png")
```
...
If you're interested in looking at the website in your browser, you can do it with `browseURL(prep_browser(school_url))`. For this example we'll skip it since we will see images of the website down below. Let's read the HTML (XML and HTML are **usually** interchangeable, so here we use `read_html`).
...
There we go! We went from literally no information at the beginning of this tutorial to interpretable and summarized information only using web data. We can see some schools in Madrid (center) as well in other regions of Spain, including Catalonia and Galicia. This marks an end to our scraping adventure but before we finish, I want to mention some of the ethical guidelines for webscraping. Scraping is extremely useful but we give headaches to other people maintaining our website of interest. Here's a list of ethical guidelines you should always follow:
